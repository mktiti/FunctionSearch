...
import com.mktiti.fsearch.backend.cache.CacheApproxLimits
...
object ContextManagerStore {
    ...
    fun init(
            storeRoot: Path,
            jclDocLocation: Path?,
            approxLimits: CacheApproxLimits
    ) {
        ...
        ContextManagerStore.artifactManager = artifactManager
        ...
        val javadocManager: DocManager = DefaultDocManager(
                cache = ArtifactDocStoreWrapper(cachedDocsStore(fsDocsStore(docsStorePath), approxLimits.docs.toKilobytes())),
                artifactInfoFetcher = artifactFetcher,
                javadocParser = jarHtmlJavadocParser
        )
